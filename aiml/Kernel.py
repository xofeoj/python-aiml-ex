...
############################################################################
# THIS SECTION WAS ADDED BY MICHAEL SAXON, THIS IS THE FIRST PART THAT HAS #
# TO BE MODIFIED IN ORDER TO ADD OUTPUT PARAMETERS.  CURRENTLY MESSAGE AND #
# EMOTION ARE THE ONLY OUTPUT PARAMETERS. TO ADD MORE, FIRST ADD THE PARAM #
# IN THE OUTPUT CLASS AND CONSTRUCTOR, THEN GO TO LINE 373 TO MODIFY THE   #
# OUTPUT PROCEDURE TO INCLUDE THE PARAMATERS YOU HAVE ADDED                #
#                                                                          #
# define an output struct for sending out the message and emotion together #
# in the respond class                                                     #
class output:
    message = ""
    emotion = ""
#                                                                          #
    def __init__(self, msg, emo):
        self.message=msg
        self.emotion=emo
#                                                                          #
...
class Kernel:
    ...
    def __init__(self):
        ...
        self._version = "PyAIML 0.8.6 with NUS output spec modifications"
    ...
    def respond(self, input, sessionID = _globalSessionID):
        ...
        ####################################################################
        # THIS SECTION WAS ADDED BY MICHAEL SAXON TO UTILIZE OUTPUT OBJECT #
        # ADD ADDITIONAL PARAMETERS HERE:                                  #
        finalEmotion = self.getPredicate("emotion", sessionID)             
        # (EXAMPLE)                                                        #
        # Say we want to add "hand_gesture" as an output parameter         #
        # finalHandGesture = self.getPredicate("hand gesture", sessionID)  #
        # finalOutputRaw = output(finalResponse, finalEmotion, finalHandGesture)
        # add all other struct parameters here and change finalOutputRaw and
        # finalOutputEncoded initialization statements accordingly         #
        #                                                                  #
        finalOutputRaw = output(finalResponse, finalEmotion)
        # release the lock and return                                      #
        self._respondLock.release()
        try: 
            finalOutputEncoded = output(finalResponse.encode(self._textEncoding), finalEmotion)
            return finalOutputEncoded
        except UnicodeError: return output(finalResponse, finalEmotion)
        ####################################################################
